...
async function getApiQuoteToken(session_token) {
    try {
        const response = await axios.get(`${API_URL}/api-quote-tokens`, {
            headers: {
...
async function main() {
    ...
    if (sessionParameters && sessionParameters.data["session-token"]){
        ...
        response = await getApiQuoteToken(sessionParameters.data["session-token"]);
        console.log("getApiQuoteToken()= ", response);
        saveJsonToFile(DX_LINK_FILE, response);
    } else {
